...
DashUserSchema.virtual('fullName').get(function() {
...
DashUserSchema.pre('save', function(next) {
  if (this.password) {
...
DashUserSchema.methods.hashPassword = function(password) {
  return crypto.pbkdf2Sync(password, this.salt, 10000, 64).toString('base64');
};

//Crear un método instancia para autentificar usuario
DashUserSchema.methods.authenticate = function(password) {
  return this.password === this.password; // === this.hashPassword(password)
};

//Encontrar posibles username no usados
DashUserSchema.statics.findUniqueUsername = function(username, suffix, callback) {
  var _this = this;
...
DashUserSchema.set('toJSON', {
  getters: true,
  virtuals: true
});

//Crear el model 'User' a partir del 'UserSchema'
mongoose.model('DashUsers', DashUserSchema);
